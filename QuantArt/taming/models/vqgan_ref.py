...
class VQModel_Ref(pl.LightningModule):
    ...
    def log_images(self, batch, **kwargs):
        ...
        return log
    def generate_style(self,batch):
        images = self.get_input(batch, self.image_key1)
        images = images.to(self.device)
        styles = self.get_input(batch, self.image_key2)
        # print(styles[0]==styles[1])
        outs = []
        for style in styles:
            curr_styles = style.repeat(len(images),1,1,1).to(self.device)
            quant_x, quant_y, quant_ref, diff_x2y, indices_y, indices_ref = self.transfer(images, curr_styles)
            out = self.decode(quant_y)
            outs.append(out)
        return outs
