...
class UnpairedImagePaths(Dataset):
    ...
    def __len__(self):
        if len(self.paths2) > 1: 
            self._length = min(len(self.paths1), len(self.paths2)) # len(paths1) 
        else: # single style in a folder
            self.paths2 = self.paths2 * len(self.paths1)
            self._length = len(self.paths1)
        return self._length
    ...
    def __getitem__(self, i):
        ...
        example["image2"], example["image2_path"] = self.preprocess_image(self.paths2[i])
